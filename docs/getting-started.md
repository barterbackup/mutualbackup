...
| `expected_node_id` | Public identity derived during initialization. A wrong unlock is rejected against this value before SQLCipher is opened. |
...
| `recovery_mode` | `false` for a normal node; `recover-init` writes `true` for a recovery-string-only recovery node. |
...
"$CLI" --socket "$LAB/single/run/control.sock" init \
  --config "$LAB/single/node.toml" \
  --data-dir "$LAB/single/state" \
  --failure-domain "single-test-disk" \
  --listen /ip4/0.0.0.0/udp/44000/quic-v1 \
...
field; its example public identity must be replaced by one generated by
`mutualbackup init`.
...
directory, recovery string, and lab failure-domain label:
...
Create a blank recovery node from only node 1's retained recovery string and the generic

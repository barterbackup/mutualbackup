...
impl P2pEventLoop {
    ...
    fn schedule_duplicate_session_collapse(&mut self, peer: PeerId, new_connection: ConnectionId) {
                ...
                .then_some((
                    ...
                    *path,
                    ...
                    self.sessions
                        .get(connection)
                        .map(|session| session.sequence)
                        .unwrap_or(if *connection == new_connection {
                            u64::MAX
                        } else {
                            0
                        }),
        ...
        let best_rank = connections
            .iter()
            .map(|(_, path, _, _)| path_preference_rank(self.tor_mode, *path))
            .min()
        ...
        let local_prefers_dialer = self.swarm.local_peer_id() < &peer;
        let preferred_direction_exists = connections.iter().any(|(_, path, dialer, _)| {
            path_preference_rank(self.tor_mode, *path) == best_rank
        ...
        for (connection, path, dialer, _) in &connections {
            if path_preference_rank(self.tor_mode, *path) == best_rank
                && *dialer != local_prefers_dialer
                && !self.connection_owns_relay_reservation(*connection)
            {
        ...
        if local_prefers_dialer {
            let survivor = connections
                .iter()
                .filter(|(connection, path, dialer, _)| {
                    path_preference_rank(self.tor_mode, *path) == best_rank
                        && *dialer
                        && !self.connection_owns_relay_reservation(*connection)
                })
                .min_by_key(|(connection, _, _, sequence)| (*sequence, *connection))
                .map(|(connection, _, _, _)| *connection);
            if let Some(survivor) = survivor {
                self.duplicate_retirement.remove(&survivor);
                for (connection, path, dialer, _) in &connections {
                    if *connection != survivor
                        && path_preference_rank(self.tor_mode, *path) == best_rank
                        && *dialer
                        && !self.connection_owns_relay_reservation(*connection)
                    {
    ...
    fn reconcile_duplicate_sessions(&mut self, peer: PeerId) {
        ...
        // candidate as the incremental trigger; the collapse routine ranks the
        // full set and retains the oldest preferred-direction survivor.
        let newest = self
            .connection_paths
            .iter()
        ...
        if !pending {
            if let Some(catalog) = &checkpoint.checkpoint.packing_catalog {
                ...
                if !uncovered.is_empty() {
                    ...
                    .await?;
                    let revision_id = Uuid::from_bytes(
                        catalog.id[..16]
                            .try_into()
...
async fn recover_from_dht_once(
    ...
    node_blocking(node.clone(), move |node| {
        if matches!(recovery_checkpoint.checkpoint.format_version, 4..=7) {
...
pub async fn audit_guild(
    ...
    let (checkpoint, roster, local_id, variable_groups) = node_blocking(node.clone(), |node| {
        ...
        let variable_groups = node
            ...
            .into_iter()
            .filter(|retained| {
                variable_group_protects_checkpoint(&checkpoint.checkpoint, &retained.group)
            })
...
async fn recover_p2p_variable_shards(
    ...
    let plans = node_blocking(node.clone(), move |node| {
        ...
        for retained in state.coding_groups {
            if !retained.group.roles.iter().any(|role| {
                matches!(role, ShardRoleV2::Information(information)
                    if !information.sector.virtual_zero
                        && sectors.contains(&information.sector.id))
            }) {
                continue;
            }
    ...
    if let Some(catalog) = &checkpoint.checkpoint.packing_catalog {
        ...
        node_blocking(node.clone(), move |node| {
            let state = node
                .dynamic_guild_state()?
                .context("packed recovery requires dynamic guild state")?;
            let assignments = recovered_catalog
                .sectors
                .iter()
                .filter_map(|descriptor| {
                    state.coding_groups.iter().find_map(|retained| {
                        retained
                            .group
                            .roles
                            .iter()
                            ...
                            .find_map(|(index, role)| {
                                matches!(role, ShardRoleV2::Information(information)
                                    if information.owner == local_id
                                        && !information.sector.virtual_zero
                                        && information.sector.id == descriptor.id
                                        && information.sector.commitment == descriptor.commitment)
                                .then_some((descriptor.clone(), retained.group.id, index as u16))
                            })
                    })
                })
                .collect::<Vec<_>>();
            for (descriptor, group_id, shard_index) in assignments {
                let bytes = node.variable_shard_for_guild(&guild_id, &group_id, shard_index)?;
                node.store_packed_sector(
                    guild_id,
                    recovered_catalog.profile,
                    PackedSector { descriptor, bytes },
                )?;
            }
            Ok(())
        })
        .await?;
        let variable_groups = node_blocking(node.clone(), |node| {
            Ok(node
                .dynamic_guild_state()?
                .context("packed recovery requires dynamic guild state")?
                .coding_groups)
            ...
            .filter(|revision| revision.value.owner == local_id)
        {
            ...
            {
                references.insert(
                    (revision.value.protected_root_id, reference.id),
                    reference.clone(),
                );
            }
        }
        for ((protected_root_id, _), reference) in references {
            let bytes = reconstruct_packed_object(
                node.clone(),
                p2p,
            ...
            let guild_id = checkpoint.checkpoint.guild_id;
            node_blocking(node.clone(), move |node| {
                node.install_repaired_information_sector(guild_id, reference, &bytes)
            })
            .await?;
        }
    }
...
#[allow(clippy::too_many_arguments)]
async fn reconstruct_packed_object(
    node: Arc<Mutex<Node>>,
    p2p: &P2pClient,
    guild_id: [u8; 32],
    ...
    reference: &SectorRef,
    variable_groups: &[mb_core::RetainedCodingGroup],
    roster: &[GuildPeer],
    deferred_holders: &Mutex<BTreeSet<NodeId>>,
) -> Result<Vec<u8>> {
    let protected_root = packing_protected_root(protected_root_id);
            ...
            descriptor.slots.iter().any(|slot| {
                slot.source().is_some_and(|source| {
                    (
                        ...
                        source.id.object_id,
                    ) == (owner, protected_root, reference.id)
                })
            })
        })
        .cloned()
    ...
    if descriptors.is_empty() {
        bail!("signed revision sector is absent from its packing catalog");
    }
    let mut sectors = Vec::with_capacity(descriptors.len());
    for descriptor in descriptors {
        let sector_id = descriptor.id;
        let local = node_blocking(node.clone(), move |node| {
        ...
        let bytes = match local {
            Ok(bytes) => bytes,
            Err(_) => {
                let (group, target_index) = variable_groups
                    .iter()
                    .find_map(|retained| {
                        retained
                            .group
                    ...
                    .context("packed sector is absent from certified variable coding")?;
    ...
    unpack_object_from_sectors(catalog, &sectors, owner, protected_root, reference.id)
        .map_err(Into::into)
}
...
pub(crate) async fn restore_snapshot_with_p2p(
    ...
    let (checkpoint, revision, roster, variable_groups) =
        ...
        .await?;
    let guild_id = checkpoint.checkpoint.guild_id;
    let packing_catalog = checkpoint.checkpoint.packing_catalog.clone();
    let revision_owner = revision.value.owner;
    let protected_root_id = revision.value.protected_root_id;
    ...
    for reference in references {
        ...
        let bytes = if let Some(catalog) = &packing_catalog {
            reconstruct_packed_object(
                node.clone(),
                p2p,
                guild_id,
                catalog,
            ...
            .await?
        } else if let Some((group, target_index)) = variable {
            reconstruct_variable_shard_from_peers(
                node.clone(),
                p2p,
                group,
...
async fn build_production_packing(
    node: Arc<Mutex<Node>>,
    p2p: &P2pClient,
    local_id: NodeId,
    guild_id: [u8; 32],
    ...
    for ((owner, protected_root, object_id), (reference, source_commitment)) in sources {
        let source = if owner == local_id {
            ...
            p2p.sector(owner, guild_id, object_id).await
        };
        let bytes = match source {
            Ok(bytes) => bytes,
            Err(source_error) => {
                let catalog = previous.cloned().context(
                    "retained source is unavailable and has no previous packing catalog",
                )?;
                node_blocking(node.clone(), move |node| {
                    let state = node
                        .dynamic_guild_state()?
                        .context("prior packed recovery requires dynamic guild state")?;
                    let descriptors = catalog
                        .sectors
                        .iter()
                        .filter(|descriptor| {
                        ...
                        .cloned()
                        .collect::<Vec<_>>();
                    let sectors = descriptors
                        .into_iter()
                        .map(|descriptor| {
                            let bytes = match node.sector_for_guild(&guild_id, &descriptor.id) {
                                Ok(bytes) => bytes,
                                Err(_) => {
                                    let (group_id, shard_index) = state
                                        .coding_groups
                                        .iter()
                                        .find_map(|retained| {
                                            retained.group.roles.iter().enumerate().find_map(
                                                |(index, role)| {
                                                    matches!(role, ShardRoleV2::Information(information)
                                                        if !information.sector.virtual_zero
                                                            && information.sector.id == descriptor.id
                                                            && information.sector.commitment == descriptor.commitment)
                                                    .then_some((retained.group.id, index as u16))
                                                },
                                            )
                                        })
                                        .context(
                                            "prior packed sector has no certified coding group",
                                        )?;
                            ...
                            Ok(PackedSector { descriptor, bytes })
                        })
                ...
                .with_context(|| {
                    format!(
                        "retained source is unavailable from its owner and the prior packed copy could not be read: {source_error}"
                    )
                })?
            }
        };
...
async fn commit_backup_job(
    ...
    .await?;
    let previous_packing_catalog = previous
        .as_ref()
        .and_then(|checkpoint| checkpoint.checkpoint.packing_catalog.clone());
    certificate.verify()?;
    ...
    coding_groups.clear();
    let packing = build_production_packing(
        node.clone(),
        p2p,
        local_id,
        guild_id,
    ...
    let checkpoint = GuildCheckpoint {
        ...
        packing_catalog: Some(packing.catalog),
    };
    checkpoint.validate()?;
    let checkpoint_hash = checkpoint.hash()?;
...
mod tests {
    ...
    #[tokio::test]
    async fn selected_close_reselects_one_of_three_retiring_duplicates_without_losing_request() {
        let temp = tempfile::tempdir().unwrap();
        let local_seed = Seed::from_bytes([106; 32]);
        let local_id = KeyMaterial::from_seed(&local_seed).node_id();
        let node = Arc::new(Mutex::new(Node::open(temp.path(), local_seed).unwrap()));
        ...
        let second_duplicate = ConnectionId::new_unchecked(1064);
        event_loop
            .connection_paths
        ...
        event_loop
            .connection_paths
            .insert(second_duplicate, (peer, P2pPath::Direct));
        ...
        event_loop
            .connection_dialers
            .insert(second_duplicate, local_prefers_dialer);
        event_loop.record_session_open(
            selected,
            peer,
            P2pPath::Direct,
        ...
        event_loop.handle_swarm_event(SwarmEvent::ConnectionClosed {
            peer_id: peer,
            connection_id: second_duplicate,
            endpoint: ConnectedPoint::Listener {
                local_addr: direct.clone(),
    ...
    #[tokio::test]
    async fn selected_close_reconciles_duplicates_when_local_peer_prefers_listener() {
        let temp = tempfile::tempdir().unwrap();
        let local_seed = Seed::from_bytes([107; 32]);
        let local_id = KeyMaterial::from_seed(&local_seed).node_id();
        let node = Arc::new(Mutex::new(Node::open(temp.path(), local_seed).unwrap()));
        let (_client, mut event_loop) = build_p2p(node, config(local_id)).unwrap();
        let peer = KeyMaterial::from_seed(&Seed::from_bytes([106; 32]))
            .node_id()
            .libp2p_peer_id()
            .unwrap();
        assert!(event_loop.swarm.local_peer_id() > &peer);

        let selected = ConnectionId::new_unchecked(1071);
        let survivor = ConnectionId::new_unchecked(1072);
        let duplicate = ConnectionId::new_unchecked(1073);
        let second_duplicate = ConnectionId::new_unchecked(1074);
        for (connection, dialer) in [
            (selected, false),
            (survivor, false),
            (duplicate, true),
            ...
            event_loop
                .connection_paths
                .insert(connection, (peer, P2pPath::Direct));
            event_loop.connection_dialers.insert(connection, dialer);
            event_loop.record_session_open(
                connection,
                peer,
                P2pPath::Direct,
        ...
        event_loop.connection_paths.remove(&selected);
        event_loop.record_session_close(selected, false);
        for connection in [survivor, duplicate, second_duplicate] {
            event_loop.duplicate_retirement.insert(
                connection,
                (peer, tokio::time::Instant::now() + Duration::from_secs(1)),
            );
        ...
        event_loop.restore_selected_transport_after_close(peer, Some(P2pPath::Direct));
    ...
    async fn repeated_multi_owner_backups_commit_over_quic() {
        ...
        let mut packed_catalogs = Vec::new();
        ...
        let final_checkpoint = nodes[0]
            ...
            .unwrap()
            .unwrap();
        assert_eq!(final_checkpoint.checkpoint.format_version, 7);
        ...
        let final_catalog = final_checkpoint
            .checkpoint
            .packing_catalog
            .as_ref()
            .unwrap();
        assert_eq!(final_catalog.revision, 4);
        assert_eq!(final_catalog.parent, Some(packed_catalogs[2].id));
        let slot_positions = |catalog: &PackedCatalog| {
            catalog
                .sectors
                .iter()
                .flat_map(|sector| {
                    sector
                        ...
                        .filter_map(move |(slot, value)| {
                            value
                                .source()
                                .map(|source| (source.id, (sector.sector_index, slot)))
                        })
                })
                .collect::<BTreeMap<_, _>>()
        };
        let first_positions = slot_positions(&packed_catalogs[0]);
        let final_positions = slot_positions(final_catalog);
        assert!(
            first_positions
                .iter()
                .all(|(source, position)| final_positions.get(source) == Some(position))
        );
        let packed_owners = final_catalog
            .sectors
            .iter()
            .flat_map(|sector| &sector.slots)
            .filter_map(|slot| slot.source().map(|source| source.id.owner))
            .collect::<BTreeSet<_>>();
        assert!(packed_owners.contains(&nodes[1].lock().unwrap().keys().node_id()));
        assert!(packed_owners.contains(&nodes[2].lock().unwrap().keys().node_id()));
        assert!(final_catalog.sectors.iter().any(|sector| {
            sector
                .slots
                .iter()
                .filter_map(|slot| slot.source().map(|source| source.id.owner))
